...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Bugs_Fixed-28-orange?style=flat-square" alt="Bugs Fixed">
</p>
...
- [Hotfixes v1.3 — 21 Bugs Fixed](#-hotfixes-v13--21-bugs-fixed)
- [Build Fixes v1.3 — 7 Errors Fixed](#-build-fixes-v13--7-errors-fixed)
...
## 🔧 Build Fixes v1.3 — 7 Errors Fixed

Sau khi hoàn thiện code, quá trình build trên Ubuntu 22.04 / GCC 12 phát sinh 7 lỗi liên quan đến toolchain compatibility. Tất cả đã được fix và verified trên 2 máy độc lập.
...
PIE enabled  ✔    cap_ipc_lock  ✔    Symbols     stripped
```

### Danh sách Build Errors

| # | File | Lỗi | Root Cause | Fix |
|---|------|-----|-----------|-----|
| ERR-A | `install.sh` | `wait_for: No record of process` | `spin_stop()` gọi `wait $PID` sau khi spinner process đã chết | Thay `wait` bằng `disown` |
| ERR-B | `CMakeLists.txt` | `_FORTIFY_SOURCE` redefined | GCC 12 Ubuntu 22.04 tự define `_FORTIFY_SOURCE=2` khi `-O3` | Thêm `-U_FORTIFY_SOURCE` + wrap trong `$<$<CONFIG:Release>:...>` |
| ERR-C | `MemoryGuard.hpp` | `volatile array::data() discards qualifiers` | `std::array` không có `volatile`-qualified `data()` overload | Đổi `volatile std::array<uint64_t, N>` → `uint64_t canary_[N]` raw array |
| ERR-D | `PrivilegeDrop.hpp` | `cannot convert brace-enclosed initializer list to ResourceLimits` | `ResourceLimits` non-aggregate → `= {}` default param thất bại | Xóa `= {}`, thêm no-arg overload `setLimits()` |
| ERR-E | `CliParser.hpp` | `makeRule not declared in scope` | `makeRule()` định nghĩa sau class `CliParser` nhưng `parseLong()` bên trong class đã gọi nó | Inject `makeRule()` trước `namespace SecFW {` |
| ERR-F | `CliParser.hpp` | `ValidationRule has no member 'regexPattern'` | Field thực tế trong `ValidationRule` là `pattern` | Đổi `.regexPattern=` → `.pattern=` trong `makeRule()` và tất cả brace-inits |
| ERR-G | `CliParser.hpp` | `0~#pragma once` — stray `#` | Vim backup artifact `0~` dính vào dòng 1 khi transfer | `sed -i '1s/.*/\#pragma once/'` |

### Verified Build Output

```
[1/2] Building CXX object ... main.cpp.o
[2/2] Linking CXX executable SecureConsoleApp
✔  Release: ~/SecureConsoleApp/build/release/SecureConsoleApp  (256KB, stripped, LTO)
✔  Debug:   ~/SecureConsoleApp/build/debug/SecureConsoleApp    (ASan + UBSan)
✔  cap_ipc_lock granted
```
...
| `InputValidator.hpp` | 354 | `ValidationRule` (fields: `pattern`, `allowedChars`, `minLen`, `maxLen`, `checkSQLi`, `noControlChars`), `Rules::USERNAME/PASSWORD/TEXT/FILE_PATH`, `validate()`, `parseInteger<T>`, `sanitize()`, `detectSQLi()`, `isValidPath()` |
...
| Tổng bug đã fix | 21 (code) + 7 (build) = **28** ✅ |
| File mới (features) | 7 ✅ |
...
| `InputValidator.hpp` | Field `regexPattern` đổi tên thành `pattern` (ERR-F build fix — unify field name) |
...
<p align="center">
  Built with ❤️ for secure C++ development — v1.3 · 28 fixes · Verified on Ubuntu 22.04
</p>
